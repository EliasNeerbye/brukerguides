...
mongoose
    .connect("mongodb://localhost:27017/brukerguides")
...
app.get("/guide/:id", async (req, res) => {
    try {
        ...
        if (!isValidGuideId(guideId)) {
            return res
                .status(400)
                .send("Invalid guide ID. Must be a 24-character hex string, 12-byte Uint8Array, or an integer.<br><br><br><a href='/'>Go home</a>");
        }
...
app.post("/saveGuide/:id", async (req, res) => {
    ...
    try {
        ...
        await Guide.findByIdAndUpdate(guideId, { title, sections: updatedSections }, { new: true, runValidators: true });
...
app.post("/deleteGuide/:id", async (req, res) => {
    ...
    try {
        ...
        const imgUrls = guide.sections.flatMap((section) => section.images.map((image) => image.url));
...
app.post("/verifyAndChange", async (req, res) => {
    ...
    try {
        ...
        if (codeExpiry[user.email]) {
            // Assuming you are still using email as a key
